...
/**
 * 检测分类是否还可以发布  精品专线、落地配、综合物流、推荐货车、生产厂家只能发布1条 货源根据vip等级来限制
 * @param $cate_id
 * @return bool
 */
...
function getNextCategory($cate_id = 0, $province_id = 0){
    ...
    if(!$cache){
        ...
        $cache = M('Category')->field('id,name,title,list_row,province_id,template_lists')->where($map)->order('sort asc')->select();
        if(!$cache){
            return false;
        }
        $new = array();
